...
// (vLLM, llama.cpp server, LM Studio, LiteLLM proxy, Ollama via its
// /v1 endpoints, OpenAI itself). "ollama" is the *native* Ollama API
// path (/api/generate, /api/embed) — only useful if you specifically
// need Ollama's native options; otherwise prefer backend=openai with
